// /opt/data/release-kalender/src/lib/games.ts
// Games Release Daten — gescrapt von VGC, GameInformer (Firecrawl)
// Aktualisiert: 25. Juni 2026

import type { ReleaseItem } from "./anilist";

// Kombinierte Daten aus VGC + GameInformer
const GAMES_DATA: RawGame[] = [
  ...
  { title: "The Elder Scrolls Online – Thieves Guild", date: "2026-07-08", platforms: ["PC", "Xbox", "PS5"] },
  ...
  { title: "Clive Barker's Hellraiser: Revival", date: "2026-10-08", platforms: ["PC", "Xbox", "PS5"] },
  ...
  { title: "SpongeBob SquarePants: Titans of the Tide", date: "2026-10-13", platforms: ["Switch"] },
...
function mapGameToRelease(g: RawGame, idx: number): ReleaseItem {
  ...
  return {
    ...
    type: "Game",
    score: null,
    url: `https://www.google.com/search?q=${encodeURIComponent(title + " game release")}`,
    description: null,
    category: "game",
  };
}
...
async function rawgSearch(query: string): Promise<ReleaseItem[]> {
  const key = process.env.RAWG_API_KEY;
  if (!key) return [];

  try {
    const res = await fetch(
      `${RAWG_BASE}/games?key=${key}&search=${encodeURIComponent(query)}&page_size=30`
    );
    if (!res.ok) return [];
    const json = await res.json();
    const results: RAWGResult[] = json.results || [];
    ...
    return results.map((r, i) => ({
      ...
      banner: null,
      date: r.released
        ? new Date(r.released).toLocaleDateString("de-DE", { year: "numeric", month: "long", day: "numeric" })
        : "TBA",
      genres: (r.platforms || []).slice(0, 4).map((p) => p.platform?.name || "").filter(Boolean),
      type: "Game",
      score: r.metacritic || null,
      url: `https://rawg.io/games/${r.id}`,
      description: r.description_raw || null,
...
async function rawgUpcoming(dateFrom?: string, dateTo?: string): Promise<ReleaseItem[]> {
  const key = process.env.RAWG_API_KEY;
  if (!key) return [];

  const today = new Date().toISOString().split("T")[0];
  const nextYear = `${new Date().getFullYear() + 1}-12-31`;
  const dates = dateFrom && dateTo ? `${dateFrom},${dateTo}` : `${today},${nextYear}`;

  try {
    const res = await fetch(
      `${RAWG_BASE}/games?key=${key}&dates=${dates}&ordering=released&page_size=40`
    );
    if (!res.ok) return [];
    const json = await res.json();
    return (json.results || []).map((r: RAWGResult, i: number) => ({
      id: `rawg-up-${r.id}`,
      title: r.name,
      titleOrig: r.name,
      image: r.background_image || "",
...
export async function fetchUpcomingGames(dateFrom?: string, dateTo?: string): Promise<ReleaseItem[]> {
  const rawg = await rawgUpcoming(dateFrom, dateTo);
...
export async function searchGames(query: string): Promise<ReleaseItem[]> {
  // Try RAWG first
  const rawg = await rawgSearch(query);
  if (rawg.length > 0) return rawg;
  // Fallback to local data
  const lower = query.toLowerCase();
  return GAMES_DATA
    .filter((g) => g.title.toLowerCase().includes(lower))
    .map(mapGameToRelease);
}
